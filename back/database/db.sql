-- Crear la base de datos (opcional: ejecuta solo si no existe)
CREATE DATABASE delivery_app;

-- Conectar a la base de datos (en psql: \c delivery_app)
-- En pgAdmin, selecciona la base de datos antes de ejecutar el resto.
...
CREATE TABLE usuarios (
    ...
    telefono VARCHAR(20),
    tipo VARCHAR(20) CHECK (tipo IN ('cliente', 'repartidor')),
...
CREATE TABLE repartidores (
    id SERIAL PRIMARY KEY,
    usuario_id INT REFERENCES usuarios(id) ON DELETE CASCADE UNIQUE,  -- UNIQUE para asegurar un registro por repartidor
    direccion_residencia TEXT,
    documento_identidad VARCHAR(50) NOT NULL,
    tipo_documento VARCHAR(20) NOT NULL CHECK (tipo_documento IN ('DNI', 'Pasaporte', 'Licencia', 'Otro')),  -- Ejemplos de tipos; ajusta según necesites
    foto VARCHAR(255)  -- Ruta o URL de la foto; usa BYTEA si prefieres almacenar binarios directamente
);
...
-- Nuevo índice para la tabla repartidores
CREATE INDEX idx_repartidores_usuario ON repartidores(usuario_id);

-- Crear vista para reportes de pedidos (útil para dashboards)
...
('Juan Pérez', 'juan@email.com', '123456789', 'cliente'),
('Ana López', 'ana@email.com', '987654321', 'repartidor'),
('María García', 'maria@email.com', '555666777', 'cliente');
...
-- Insertar datos de ejemplo para la nueva tabla repartidores (para el repartidor Ana López, id=2)
