...
function ClientDashboard() {
  ...
  const handleOpenRating = async (order) => {
    try {
      ...
      const driverRes = await axios.get(`${API_BASE_URL}/client/order-driver/${order.id}`, {
        withCredentials: true,
      });
      ...
      setPedidoPorCalificar({ id: order.id, nombre_conductor: "tu repartidor" });
    }
  };
              ...
              <div key={order.id} className="order-card-modern">
                ...
                <div style={{ padding: "20px 10px 10px 10px" }}>
                   ...
                   <div style={{ display: "flex", justifyContent: "space-between", position: "relative", marginBottom: "8px" }}>
                    {[1, 2, 3, 4].map((step) => (
                      <div key={step} style={{ width: "14px", height: "14px", borderRadius: "50%", background: getStepLevel(order.status) >= step ? "#007bff" : "#cbd5e1", zIndex: 2 }} />
                    ))}
                ...
                <div className="order-footer" style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  <button onClick={() => handleOpenDetails(order.id, order.status)} className="btn-outline" style={{ flex: 1 }}>
                    Ver Conductor
                  </button>
